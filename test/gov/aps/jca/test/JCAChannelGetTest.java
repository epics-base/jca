...
public class JCAChannelGetTest extends TestCase {
    ...
    private void internalGetTest(DBRType type, int count) throws CAException, TimeoutException, InterruptedException {
    ...
    private void checkDBR(DBRType type, int count, DBR dbr) {
        ...
        // TODO tmp check
        if (type.getValue() == DBR_PutAckS.TYPE.getValue() || type.getValue() == DBR_PutAckT.TYPE.getValue()) {
            // what to do here...
        } else
    ...
    public void testGets() throws CAException, TimeoutException, InterruptedException, IOException {
        ...
        // this test is done in JCAChannelPutTest
        // internalGetTest(DBR_STSACK_String.TYPE, 1);

        /*
         * // TODO type code 31 (CTRL) - there is not predefined type // also
         * not possible to retrieve value... see at
         * com.cosylab.epics.caj.impl.DBRDecoder.getDBR(DBRDecoder.java:81)
         * //internalGetTest(DBR_LABELS_Enum.TYPE, 1); CAJChannel cchannel =
         * (CAJChannel)channel;
         * 
         * GetListenerImpl listener = new GetListenerImpl(); new
         * ReadNotifyRequest(cchannel, listener, null, cchannel.getTransport(),
         * cchannel.getServerChannelID(), 31, 1).submit(); synchronized
         * (listener) { context.flushIO(); listener.wait(3000); }
         * assertEquals(CAStatus.NORMAL, listener.status);
         * 
         * // dirty (copy from checkDBR method) final int count = 1; final DBR
         * dbr = listener.value; assertEquals(count, dbr.getCount());
         * assertEquals(count, Array.getLength(dbr.getValue()));
         * 
         * checkValue(dbr.getValue(), count); if (dbr instanceof STS) { STS sts
         * = (STS)dbr; assertEquals(Status.NO_ALARM, sts.getStatus());
         * assertEquals(Severity.NO_ALARM, sts.getSeverity()); } if (dbr
         * instanceof TIME && !dbr.isGR()) { TIME time = (TIME)dbr; // 5 sec
         * long diff = Math.abs(new TimeStamp().secPastEpoch() -
         * time.getTimeStamp().secPastEpoch()); assertTrue(diff < 100); } if
         * (dbr instanceof GR) { GR gr = (GR)dbr; assertEquals(100,
         * gr.getUpperDispLimit().doubleValue(), 0.0001); assertEquals(5,
         * gr.getUpperAlarmLimit().doubleValue(), 0.0001); assertEquals(3,
         * gr.getUpperWarningLimit().doubleValue(), 0.0001); assertEquals(-3,
         * gr.getLowerWarningLimit().doubleValue(), 0.0001); assertEquals(-5,
         * gr.getLowerAlarmLimit().doubleValue(), 0.0001); assertEquals(-100,
         * gr.getLowerDispLimit().doubleValue(), 0.0001); assertEquals("units",
         * gr.getUnits()); } if (dbr instanceof CTRL) { CTRL ctrl = (CTRL)dbr;
         * assertEquals(100, ctrl.getUpperCtrlLimit().doubleValue(), 0.0001);
         * assertEquals(-100, ctrl.getLowerCtrlLimit().doubleValue(), 0.0001); }
         * if (dbr instanceof PRECISION) { PRECISION prec = (PRECISION)dbr;
         * assertEquals(2, prec.getPrecision()); }
         */
    }
